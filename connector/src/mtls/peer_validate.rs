...
pub fn authorize_peer_certificate(
    ...
    authorize_peer_certificate_der(expected_workspace_id, &cert_der, revocation_cache)
}
...
pub fn authorize_peer_certificate_der(
    ...
    if peer_cert_der.is_empty() {
        return Err(PeerValidationError::InvalidInput(
            "peer certificate der is required",
        ));
    }

    let san = extract_san_claims(peer_cert_der)?;
    let claimed_device_id = select_device_for_workspace(expected_workspace_id, &san)?;
    let fingerprint = sha256_fingerprint_hex(peer_cert_der);
